...
const getReserveAssetAddresss = (network: eNetwork) => ({
  vDOT: marketConfigs.StarlayConfig.ReserveAssets[network]['vDOT'],
});

// 1: deploy new assets
task('external:deploy-vdot:deploy-new-asset').setAction(async ({ verify }, localBRE) => {
  ...
  await DRE.run('external:deploy-new-asset', { symbol: 'vDOT', pool: 'Starlay' });
});
// 2: initialize & configure
task('external:deploy-vdot:init-and-configure').setAction(async ({ verify }, localBRE) => {
  if (!(SUPPORTED_NETWORK as ReadonlyArray<string>).includes(localBRE.network.name))
    throw new Error(`Support only ${SUPPORTED_NETWORK} ...`);
  setDRE(localBRE);
...
task('external:deploy-vdot:setup-oracles').setAction(async ({ verify }, localBRE) => {
  if (!(SUPPORTED_NETWORK as ReadonlyArray<string>).includes(localBRE.network.name))
    throw new Error(`Support only ${SUPPORTED_NETWORK} ...`);
  setDRE(localBRE);
  ...
  await priceOracle.submitPrices([reserveAssetAddresss.vDOT], [INITIAL_PRICES.vDOT]);
  ...
  await priceOracleDiaImpl.setAssetSources([reserveAssetAddresss.vDOT], [SYMBOLS.vDOT]);
});
