...
contract UniswapLiquiditySwapAdapter is BaseUniswapAdapter {
   ...
   * The user should give this contract allowance to pull the LTokens in order to withdraw the underlying asset and
  ...
  struct SwapAndDepositLocalVars {
    uint256 i;
    uint256 lTokenInitiatorBalance;
  ...
  function swapAndDeposit(
    ...
    for (vars.i = 0; vars.i < assetToSwapFromList.length; vars.i++) {
      vars.lToken = _getReserveData(assetToSwapFromList[vars.i]).lTokenAddress;

      vars.lTokenInitiatorBalance = IERC20(vars.lToken).balanceOf(msg.sender);
      vars.amountToSwap = amountToSwapList[vars.i] > vars.lTokenInitiatorBalance
        ? vars.lTokenInitiatorBalance
  ...
  function _swapLiquidity(
    ...
    vars.lToken = _getReserveData(assetFrom).lTokenAddress;

    vars.lTokenInitiatorBalance = IERC20(vars.lToken).balanceOf(initiator);
    vars.amountToSwap = swapAllBalance && vars.lTokenInitiatorBalance.sub(premium) <= amount
      ? vars.lTokenInitiatorBalance.sub(premium)
      : amount;
    ...
    _pullLToken(assetFrom, vars.lToken, initiator, vars.amountToPull, permitSignature);
