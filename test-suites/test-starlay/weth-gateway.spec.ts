...
makeSuite('Use native ETH at LendingPool via WETHGateway', (testEnv: TestEnv) => {
  ...
  it('Withdraw WETH - Full', async () => {
    const { users, lWETH, wethGateway, pool } = testEnv;
  ...
  it('Borrow ETH via delegateApprove ETH and repays back', async () => {
    const { users, wethGateway, lWETH, weth, helpersContract, pool } = testEnv;
    const borrowSize = parseEther('1');
    const user = users[2];
    const { variableDebtTokenAddress } = await helpersContract.getReserveTokensAddresses(
      weth.address
    ...
    await waitForTx(
      await wethGateway.connect(user.signer).borrowASTR(pool.address, borrowSize, '2', '0')
    );
